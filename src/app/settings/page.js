...
const btnDestructive = {
  backgroundColor: 'transparent', color: '#721c24',
  fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: '500',
  padding: '8px 16px', borderRadius: '4px', border: '1px solid #dc3545', cursor: 'pointer',
}
...
// Product table: desktop = SKU, NAME, REORDER THRESHOLD, ACTIONS
//                mobile  = SKU, NAME, ACTIONS
const PROD_COLS = [
  { key: 'sku',       label: 'SKU',               mobile: true  },
  { key: 'name',      label: 'Name',              mobile: true  },
  { key: 'threshold', label: 'Reorder Threshold', mobile: false },
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '14px 0', borderBottom: `1px solid ${c.softDune}` }}>
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: checked ? c.umber : c.softDune, cursor: 'pointer',
          position: 'relative', transition: 'background-color 0.2s ease',
          flexShrink: 0, marginLeft: '24px', marginTop: '2px',
        }}
        aria-label={label}
      >
        <span style={{
...
function DeleteModal({ product, confirmText, onTextChange, onConfirm, onCancel, isMobile }) {
  if (!product) return null
  const canConfirm = confirmText === 'DELETE'
  return (
    <>
      <div onClick={onCancel} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(37,37,37,0.28)', zIndex: 200 }} />
      <div style={{
        position: 'fixed',
        top: isMobile ? '0' : '50%',
        left: isMobile ? '0' : '50%',
        ...
        zIndex: 300, fontFamily: 'var(--font-body)',
        display: 'flex', flexDirection: 'column',
      }}>
        <div style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: '500', color: '#252525', marginBottom: '12px' }}>
          Delete {product.name}?
        </div>
        <div style={{ backgroundColor: '#f8d7da', border: '1px solid #f5c6cb', borderRadius: '4px', padding: '12px 14px', marginBottom: '20px', fontSize: '13px', color: '#721c24', lineHeight: 1.6 }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <input type="text" value={confirmText} onChange={e => onTextChange(e.target.value)} placeholder="DELETE" autoFocus style={{ ...inputStyle, borderColor: confirmText && !canConfirm ? '#dc3545' : c.softDune }} />
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={onConfirm} disabled={!canConfirm} style={{ ...btnDestructive, backgroundColor: canConfirm ? '#dc3545' : '#f8d7da', color: canConfirm ? '#fff' : '#721c24', borderColor: canConfirm ? '#dc3545' : '#f5c6cb', cursor: canConfirm ? 'pointer' : 'not-allowed', border: 'none' }}>
            Confirm Delete
          </button>
...
function AddProductPanel({ show, form, onFormChange, onSave, onCancel, isMobile }) {
  const canSave = form.name.trim() && form.sku.trim() && form.threshold && form.confirmed
  return (
    <>
      {show && (
        <div onClick={onCancel} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(37,37,37,0.18)', zIndex: 200 }} />
      )}
      <div style={{
        ...
        backgroundColor: '#fff',
        borderLeft: isMobile ? 'none' : `1px solid ${c.softDune}`,
        zIndex: 300, display: 'flex', flexDirection: 'column',
        transform: show ? 'translateX(0)' : 'translateX(100%)',
        transition: 'transform 0.26s cubic-bezier(0.4,0,0.2,1)',
      }}>
        <div style={{ padding: '28px 28px 20px', borderBottom: `1px solid ${c.softDune}`, flexShrink: 0 }}>
          <div style={{ fontFamily: 'var(--font-heading)', fontSize: '20px', fontWeight: '500', color: '#252525' }}>Add Product</div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          {[
            { key: 'name',      label: 'Product Name *',      type: 'text',   placeholder: 'e.g. Face Serum' },
            { key: 'sku',       label: 'SKU *',               type: 'text',   placeholder: 'e.g. AR-006' },
            { key: 'threshold', label: 'Reorder Threshold *', type: 'number', placeholder: 'e.g. 150' },
            { key: 'shopifyId', label: 'Shopify Product ID',  type: 'text',   placeholder: 'Add when Shopify is connected' },
          ].map(f => (
            <div key={f.key} style={{ marginBottom: '18px' }}>
              <label style={fieldLabel}>{f.label}</label>
              <input type={f.type} style={inputStyle} placeholder={f.placeholder} value={form[f.key]} onChange={e => onFormChange(f.key, e.target.value)} min={f.type === 'number' ? '1' : undefined} />
            </div>
          ))}
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer', marginBottom: '8px' }}>
            <input type="checkbox" checked={form.confirmed} onChange={e => onFormChange('confirmed', e.target.checked)} style={{ marginTop: '2px', width: '15px', height: '15px', flexShrink: 0, cursor: 'pointer', accentColor: c.umber }} />
        ...
        <div style={{ padding: '20px 28px', borderTop: `1px solid ${c.softDune}`, display: 'flex', gap: '10px', flexShrink: 0 }}>
          <button onClick={onSave} disabled={!canSave} style={{ ...btnPrimary, opacity: canSave ? 1 : 0.45, cursor: canSave ? 'pointer' : 'not-allowed' }}>
            Save Product
          </button>
          <button onClick={onCancel} style={btnSecondary}>Cancel</button>
        </div>
...
export default function SettingsPage() {
  ...
  function handleSaveProduct() {
    setProductList(prev => [...prev, { id: addForm.sku.trim().toUpperCase(), name: addForm.name.trim(), reorderThreshold: Number(addForm.threshold) }])
    setShowAddPanel(false)
    setAddForm(emptyAddForm)
  }

  const visibleProdCols = PROD_COLS.filter(c => !isMobile || c.mobile)
    ...
    <div>
      ...
      <div style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '16px' }}>
          ...
          <button onClick={() => setShowAddPanel(true)} style={btnPrimary}>+ Add Product</button>
        </div>

        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-body)' }}>
          <thead>
            <tr>
              {visibleProdCols.map(col => (
                <th key={col.key} style={{ ...thStyle, paddingRight: col.key === 'actions' ? '0' : '16px' }}>{col.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {productList.map(p => (
              <tr key={p.id} style={{ backgroundColor: 'transparent' }}>
                {visibleProdCols.map(col => {
                  const base = { ...tdStyle, paddingRight: col.key === 'actions' ? '0' : '16px' }
                  if (col.key === 'sku')       return <td key="sku"       style={{ ...base, fontWeight: '600', color: c.umber, fontFamily: 'monospace', fontSize: '12px' }}>{p.id}</td>
                  if (col.key === 'name')      return <td key="name"      style={base}>{p.name}</td>
                  if (col.key === 'threshold') return <td key="threshold" style={{ ...base, fontWeight: '600', color: '#252525' }}>{p.reorderThreshold.toLocaleString()} units</td>
                  if (col.key === 'actions')   return <td key="actions"   style={base}><button onClick={() => { setDeleteModal(p); setDeleteConfirmText('') }} style={btnDestructive}>Delete</button></td>
                  return null
                })}
              </tr>
            ))}
      ...
      <div style={{ border: `1px solid ${c.softDune}`, borderRadius: '4px', padding: '24px', marginBottom: '32px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <input type="email" value={email.alertEmail} onChange={e => setEmail({ ...email, alertEmail: e.target.value })} style={{ ...inputStyle, maxWidth: isMobile ? '100%' : '360px' }} />
        </div>
        ...
        {email.weeklyDigest && (
          <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: `1px solid ${c.softDune}` }}>
            ...
            <select value={email.digestDay} onChange={e => setEmail({ ...email, digestDay: e.target.value })} style={{ ...inputStyle, maxWidth: isMobile ? '100%' : '200px' }}>
      ...
      <div style={{ border: `1px solid ${c.softDune}`, borderRadius: '4px', padding: '24px' }}>
        ...
        <div style={{ backgroundColor: c.vanilla, border: `1px solid ${c.softDune}`, borderRadius: '4px', padding: '16px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ width: '36px', height: '36px', backgroundColor: c.softDune, borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>🛍</div>
          <div>
      ...
      <AddProductPanel
        ...
        onCancel={() => { setShowAddPanel(false); setAddForm(emptyAddForm) }}
        isMobile={isMobile}
      />
    </div>
  )
